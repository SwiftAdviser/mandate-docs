...
| `action_blocked` | Action type blocked by policy | Update allowed actions in policy |
| `selector_blocked` | Function selector blocked | Update allowed selectors in policy |
| `outside_schedule` | Transaction outside allowed hours | Wait for the schedule window to open |
...
| `aegis_critical_risk` | Address flagged as critical risk | Do not transact with this address. HIGH risk triggers approval instead. |
