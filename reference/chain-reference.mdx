...
Mandate supports 10 chains across 3 ecosystems: EVM, Solana, and TON. Your runtime key prefix determines which environment (mainnet vs testnet) you can use.
...
| BNB Chain | `56` | `0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d` | 18 | `mndt_live_*` |
| BNB Testnet | `97` | BNB testnet USDC | 18 | `mndt_test_*` |

### Solana chains

| Chain | Chain ID | USDC Address | Decimals | Key Prefix |
|-------|----------|-------------|----------|------------|
| Solana | `solana` | `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v` | 6 | `mndt_live_*` |
| Solana Devnet | `solana-devnet` | `EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v` | 6 | `mndt_test_*` |

### TON chains

| Chain | Chain ID | USDT Address | Decimals | Key Prefix |
|-------|----------|-------------|----------|------------|
| TON | `ton` | `EQCxE6mUtQJKFnGfaROTKOt1lZbDiiX1kCixRv7Nw2Id_sDs` | 6 | `mndt_live_*` |
| TON Testnet | `ton-testnet` | TON testnet USDT | 6 | `mndt_test_*` |

<Note>
Non-EVM chains use string-based chain IDs (`"solana"`, `"ton"`) instead of numeric IDs. Address formats and comparison rules differ per ecosystem: EVM addresses are case-insensitive, Solana and TON addresses are case-sensitive.
</Note>
...
- **`mndt_test_*`** keys work with testnet chains (Sepolia, Base Sepolia, BNB Testnet, Solana Devnet, TON Testnet). Use these for development and testing. Testnet usage is free.
- **`mndt_live_*`** keys work with mainnet chains (Ethereum, Base, BNB, Solana, TON). Use these for production deployments.
...
The TypeScript SDK exports constants for EVM chains. Non-EVM chains (Solana, TON) require passing string chain IDs directly.
...
<CardGroup cols={2}>
  ...
  <Card title="Register Agent" icon="user-plus" href="/guides/register-agent">
    Set up your agent with the right chain and key prefix.
  </Card>
</CardGroup>
