...
<CardGroup cols={3}>
  <Card title="Intent-aware decisions" icon="brain">
    The `reason` field captures what session keys miss. Your agent states its purpose for every transaction. The policy engine scans for manipulation, anomalies, and policy violations before the wallet is called.
  </Card>
  <Card title="Risk prevention" icon="shield-halved">
    14 validation checks run on every transaction: circuit breaker status, schedule windows, address allowlists, blocked actions, per-transaction limits, daily and monthly quotas, risk screening, reputation scoring, reason scanning, and approval thresholds. A circuit breaker can halt all agent activity instantly.
  </Card>
  <Card title="Immutable audit log" icon="scroll">
    Every transaction is logged with its stated purpose, validation result, policy evaluation trace, and on-chain confirmation. You get a complete record of what your agent did, why it did it, and whether it was approved.
  </Card>
</CardGroup>
