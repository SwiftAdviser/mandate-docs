...
The intent hash is a keccak256 hash of canonical transaction parameters that ensures the Mandate server and the agent agree on exactly what was validated. Both sides compute the hash from the same inputs; a mismatch blocks the transaction with `intent_hash_mismatch`. This mechanism applies only to raw validation flows. Action-based validation does not require an intent hash.
