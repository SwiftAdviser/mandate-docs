...
An intent represents a validated intent tracked through its full lifecycle. When an agent calls the Mandate API to validate a transaction, Mandate creates an intent record and tracks it from creation through signing, broadcast, and on-chain confirmation. Each intent has a unique ID, a state, and an expiration time.
...
Intents follow a strict state machine with 10 possible states. The entry point depends on which validation endpoint the agent uses: preflight validation creates a `preflight` intent, action-based validation creates an `allowed` intent, while raw validation creates a `reserved` intent. From there, the intent progresses through approval (if required), broadcast, and confirmation.
...
|-------|-------------|-----|----------|
| `preflight` | Validated via `/validate/preflight` alias, lightweight check without quota reservation | 30 min | No |
...
|------------|---------|-------|
| `[new]` to `preflight`, `reserved`, or `allowed` | Agent calls validate endpoint | Agent |
