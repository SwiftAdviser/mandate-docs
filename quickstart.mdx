...
The fastest path. One command, zero code changes. The plugin intercepts all wallet transactions Claude executes and validates them against your policy.
...
That's it. The plugin auto-gates every Bash transaction command and MCP payment tool. If Claude tries to send tokens without validating first, the plugin blocks it.

**What you get:**
- Two-phase enforcement (PreToolUse gate + PostToolUse token recording)
- Auto-scan on startup (finds unprotected wallet calls in your codebase)
- Works with any wallet: Bankr, Locus, Sponge, direct RPC, MCP tools
