...
```typescript
try {
  // Use the agent with Mandate tools
  const result = await agent.execute('Transfer 5 USDC to 0xRecipient');
} catch (err) {
  if (err.message.includes('daily_quota_exceeded')) {
    console.log('Daily limit reached, will retry tomorrow');
  }
}
```
...
```typescript
// Policy block:
// "Transfer blocked by Mandate policy: daily_quota_exceeded"
